...
async function getTimetable(semesterId = 'VL20252601') {
  try {
    ...
    const slotTimes = {
      ...
      // Lab slots - Morning (only odd numbered)
      ...
      // Lab slots - Afternoon (only odd numbered)
    ...
    rows.each((i, row) => {
      ...
      const courseCodeTitle = $(cells[2]).text().trim();
      const slotVenue = $(cells[7]).text().replace(/(\r\n|\n|\r)/gm, '').trim();
      const facNameSchool = $(cells[8]).text().replace(/(\r\n|\n|\r)/gm, '').trim();
      
      if (!courseCodeTitle) return;
      
      const codeTitle = courseCodeTitle.split('-');
      const courseCode = codeTitle[0]?.trim() || '';
      const courseTitle = codeTitle[1]?.trim() || '';
      
      const slotVenueParts = slotVenue.split('-');
      const slot = slotVenueParts[0]?.trim() || '';
      const venue = slotVenueParts[1]?.trim() || '';
      
      const facParts = facNameSchool.split('-');
      const facName = facParts[0]?.trim() || '';
      
      if (courseCode && slot && venue !== 'NIL') {
        courses.push({ courseCode, courseTitle, slot, venue, facName });
        
        // Map to schedule
        const slots = slot.split('+');
        slots.forEach(s => {
          const slotInfo = slotTimes[s];
          if (slotInfo) {
    ...
    ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].forEach(day => {
      if (schedule[day].length > 0) {
        ...
        schedule[day].forEach(cls => {
          const icon = cls.slot.startsWith('L') ? '🔬' : '📚';
          const shortTitle = cls.courseTitle.length > 35 ? cls.courseTitle.substring(0, 32) + '...' : cls.courseTitle;
          console.log(`${icon} ${cls.time.padEnd(22)} │ ${cls.courseCode.padEnd(10)} - ${shortTitle.padEnd(35)}`);
          console.log(`   📍 ${cls.venue.padEnd(18)} │ 👨‍🏫 ${cls.faculty}`);
        });
      }
    });
    ...
    console.log(`📊 Total classes this week: ${totalClasses}`);
...
async function getAcademicCalendar(semesterId = 'VL20252601') {
  try {
    ...
    // Step 4: Fetch calendar for each month
    const months = [
    ...
    for (const month of months) {
      try {
        ...
        console.log(`   Response length: ${res.data.length}`);
        console.log(`   Tables found: ${$('table.calendar-table').length}`);
        ...
        console.log(`⚠️  Could not fetch ${month.name} calendar: ${error.message}`);
        calendar[month.name] = [];
      }
    }
    ...
    for (const [monthName, events] of Object.entries(calendar)) {
      if (events.length > 0) {
        console.log(`\n${'═'.repeat(100)}`);
        ...
        events.forEach(event => {
          ...
          const icon = event.event.includes('First Instructional') ? '🎯' :
                       eventLower.includes('instructional') ? '📚' :
                       eventLower.includes('holiday') ? '🏖️' :
                       eventLower.includes('exam') || eventLower.includes('cat') || eventLower.includes('fat') ? '📝' :
                       eventLower.includes('registration') ? '📋' :
                       eventLower.includes('vacation') || eventLower.includes('break') ? '🌴' :
                       eventLower.includes('festival') ? '🎉' :
                       eventLower.includes('non-instructional') || eventLower.includes('no class') ? '🚫' : '📌';
          
          const formattedDay = monthName.substring(0, 3) + ' ' + String(event.day).padStart(2, '0');
          const noteText = event.note ? ` (${event.note})` : '';
          
          console.log(`${icon} ${formattedDay}: ${event.event}${noteText}`);
        });
      } else {
        console.log(`\n${'═'.repeat(100)}`);
        console.log(`📅 ${monthName} 2025`);
        console.log('═'.repeat(100));
        console.log('⚠️  No events found for this month');
      }
    }
    ...
    console.log(`   📚 Instructional Days: ${totalInstructionalDays}`);
    console.log(`   🚫 Non-Instructional Days: ${totalNonInstructionalDays}`);
    console.log(`   🏖️  Holidays: ${totalHolidays}`);
    console.log(`   📝 Exam Days: ${totalExams}`);
